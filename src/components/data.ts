export type DataRecord = { x: number; y: number, y2: number, yValueFormatted: string, y2ValueFormatted: string, type: string }
...
export const data = (x: number, displayType: keyof typeof DisplayType = DisplayType.Anually, type: string) : DataRecord[] => {
  ...
  return Array.from(Array(n).keys()).map((x, i) => {
    ...
    return { x: i + 1, y: yValueDivided, y2: y2ValueDivided, yValueFormatted, y2ValueFormatted, type }
  })
}