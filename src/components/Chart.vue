...
const tickFormatX = (tick: number): string => {
  if (props.tab == DisplayType.Monthly) return String(tick);
...
const triggers = {
  [StackedBar.selectors.bar]: (d: DataRecord) => {
    ...
    const lineAccentColor =
      d.type === 'income' ? 'bg-greenAccent' : 'bg-grayAccent';
    return `
        ...
        <div class="p-6 py-3">
            <div class="flex items-center">
              <div class="flex flex-col">
                <span class="text-xs text-gray-500 mb-">${typeText} · ${
      d.x
    } ${monthText} 2024</span>
                <div class="flex items-center">
...
const events = {
  [StackedBar.selectors.barGroup]: {
    mouseover: (_: any, e: MouseEvent) => {
      ...
      const targetElement: HTMLElement | null = (e.target as HTMLElement)
        ?.parentElement;
...
<template>
  <div id="chart" :class="`chart-type-${props.tab.toLowerCase()}`">
    ...
    >
      ...
      <VisTooltip
        :triggers="triggers"
        :vertical-placement="Position.Center"
    ...
    <div class="flex items-center h-2">
      <div class="w-[56px]">
        <span
          style="
            color: var(--vis-axis-tick-label-color);
            font-size: 14px;
            padding-left: 13px;
          "
          >$ 0</span
