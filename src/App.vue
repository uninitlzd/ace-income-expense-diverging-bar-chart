...
const chartData = ref<DataRecord[]>(
  data(month.value || 1, tabs.value, 'income')
);
const chartDataExpense = ref<DataRecord[]>(
  data(month.value || 1, tabs.value, 'expense')
);
...
<template>
  <Card
    style="width: 1000px; min-height: 500px; margin: 100px auto"
      ...
      <div class="flex">
        <div class="w-full flex items-center">
          ...
          <div class="flex legend">
            <div class="mr-5 text-sm flex items-center">
              <span class="legend-icon inline-block bg-greenAccent mr-2"></span>
              Income
            </div>
            <div class="text-sm flex items-center">
              <span class="legend-icon inline-block bg-grayAccent mr-2"></span>
              Expense
            </div>
          </div>
        </div>
        <div class="w-8/12 flex justify-end">
          ...
          >
            <TabsList class="w-full">
              <TabsTriggerSelect
                :value="DisplayType.Monthly"
                v-model="month"
                @update:model-value="onMonthChange"
              >
              </TabsTriggerSelect>
              <TabsTrigger :value="DisplayType.Quarter"> Quarter </TabsTrigger>
              <TabsTrigger :value="DisplayType.Anually"> Anually </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      </div>
